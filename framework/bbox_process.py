...
def pred_det_var(anchors, cls_pred, regr_pred, var, C, step=1):
	if step == 1:
		scores = cls_pred[0, :, :]
	elif step == 2:
		scores = anchors[:, -1:] * cls_pred[0, :, :]
	...
	scores = scores[keep]
	var = var[keep, :]
	return proposals, scores, var

def pred_det_var_new(anchors, cls_pred, regr_pred, var, C, step=1):
	if step == 1:
		scores = cls_pred[0, :, :]
		pre = anchors[:, -1:]
	elif step == 2:
	...
	pre = pre[keep]
	var = var[keep, :]
	return proposals, scores, var, pre